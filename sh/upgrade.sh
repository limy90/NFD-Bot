...
# 定义日志文件和戳
LOG_FILE="upgrade_log_date +%Y%m%d%H%M).txt"
...
if [ -z "$UPGRADE_INFO" ]; then
 ...
 echo -e "\n${GREEN}回滚成功，系统已回到升级前的状态。${NC}" | tee -a "$LOG_FILE"
                ...
                echo -e "\n${RED}回滚失败！尝试修复配置问题...${NC}" | tee -a "$LOG_FILE"
                dpkg --configure -a >> "$LOG_FILE" 2>&1
                if [ $? -eq 0 ]; then
                    echo -e "\n${GREEN}修复成功，请继续检查系统状态。${NC}" | tee -a "$LOG_FILE"
                else
                    echo -e "\n${RED}无法修复配置问题，请手动检查！${NC}" | tee -a "$LOG_FILE"
                fi
            fi
        fi
    fi
...
# 提示用户是否删除脚本和日志文件
read -p "[$TimeStamp] 升级完成或未执行。是否删除此脚本和日志？(Y/n) " DELETE
