...
if [[ "\$0" == *bash* || "\$0" == *sh* ]]; then
    echo -e "错误：请直接执行脚本（例如 ./aptup.sh），不要使用 source 或 . 命令！"
    exit 1
fi
...
if [ -z "$UPGRADE_INFO" ]; then
    ...
    if [[ "$DELETE" =~ ^[Yy]$ ]]; then
        ...
        if [ -f "/root/aptup.sh" ]; then
            rm -f "/root/aptup.sh" && echo -e "${GREEN}脚本已删除。${NC}"
        else
            echo -e "${RED}警告：固定脚本路径 /root/aptup.sh 不存在！${NC}"
        fi
